  ...
  (lambda (varname value env)
    (cond
      ...
      ((equal? (caar env) 'global)
       (cons (list (list varname value)) env)); ((varname value)) (a 1)...
      (else (let*
          ;((x 8) (y 9)) (a 1) (b 2) (c 5)) <- (z 10)
  ...
  (lambda (parsed-list-exp env)
    (cond
      ...
      ((equal? (caar parsed-list-exp) 'assign-exp)
       ;parsed-list-exp: ((assign x(num-exp 8)) (print-exp (var-exp a)))
       ;varname: x <- (cadr (car parsed-list-exp))
       ;value:8 <- (car (cddr (car parsed-list-exp)
       (run-block-exp
        (cdr parsed-list-exp)
        (run-assign-exp
